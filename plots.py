...
def make_dataset_stats_plots(path=PROJECT_DIR / "stats.json"):
    ...
    for name, characteristic in zip(("Pol_", "Comp_", "Unit_"), (polarities, ccat, ucat)):
...
def make_results_plots(path=PROJECT_DIR / "predictions.csv", dataset_stats=PROJECT_DIR / "stats.json"):
    ...
    wrong_responses = predictions_data[predictions_data['truth'] != predictions_data['preds']]
    wrong_categories = wrong_responses['category']
    wrong_category_components = [com for cat in wrong_categories for com in cat.split('#')]
...
def plot_cross_validation_results_all_models():
    stats = json.load(r"C:\Users\dioni\ekpa-programming-absa\final_results.json")
    names = list(stats.keys())
    accuracies = [metrics[1] for key, metrics in stats]
